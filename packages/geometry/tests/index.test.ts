...
test("Orientation round-trips through matrices and composes transforms", () => {
  ...
  expect(Orientation.fromMat3(orientation.toMat3()).equals(orientation)).toBe(true);
  ...
  expect(
    Orientation.RotateX90.multiplyMat3(Orientation.Identity.toMat3()).equals(Orientation.RotateX90),
  ).toBe(true);
});
